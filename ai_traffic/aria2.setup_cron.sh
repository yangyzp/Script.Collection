...
# ============================================================
# 修复说明（v2）：
# 原脚本在 set -eo pipefail 下有两个隐性退出点：
#   1) 空 crontab 时，crontab -l 退出码=1
#   2) grep -v 把所有行过滤掉时，最后一个 grep 退出码=1
# 两者都会被 pipefail 捕获，导致脚本在此处直接退出，
# 后续"添加新任务"和"✅ 定时任务已创建"都不会执行。
# v2 方案：用 set +e 临时关闭 errexit，整段操作完再恢复，
# 比逐条加 || true 更干净，也不会漏掉 grep 的返回码。
# ============================================================
set +e
# 删除旧的 AI_TRAFFIC 任务（用标记精准匹配）
( crontab -l 2>/dev/null || true ) | grep -v "#AI_TRAFFIC_DAY" | grep -v "#AI_TRAFFIC_NIGHT" | grep -v "#AI_TRAFFIC_DAWN" | crontab -
# 添加新任务
(
...
echo "  删除AI流量任务：( crontab -l 2>/dev/null || true ) | grep -v AI_TRAFFIC | crontab -"
echo "============================================"
