...
echo " 并发 ${CONCURRENCY}，峰值约需 ${NEED_GB}GB"
echo "============================================"
...
if [ "$SHM_AVAIL" -lt "$(( NEED_GB * 1024 * 1024 ))" ]; then
    ...
    echo ""
    echo "解决方案："
    echo "   1. 减少 CONCURRENCY（如改成 5，需 ~5GB）"
    echo "   2. 改用磁盘版（下载到 /var/tmp/ai_traffic）"
    echo "   3. 增大 /dev/shm：mount -o remount,size=20G /dev/shm"
    echo ""
    exit 1
fi
echo "✅ 内存充足"
