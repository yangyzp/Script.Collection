...
# Linux 生产级智能优化脚本 v3.3.0 (低CPU优化版 | 带宽满速+低重传+低占用)
# 优化：硬件档位自动适配、低配CPU开销大幅降低、折中BDP平衡速度与重传
...
echo -e "${BLUE}=======================================================${NC}"
echo -e "${BLUE}  Linux 生产级智能优化脚本 v3.3.0 | 低CPU优化版${NC}"
echo -e "${BLUE}=======================================================${NC}"
echo ""
...
echo -e "${YELLOW}5. TCP缓冲区自动计算（折中BDP版：平衡速度与重传）${NC}"
TCP_BUFFER_CONFIG=""
AUTO_CALC_DONE=0
...
# 代理场景小幅叠加系数
if [[ "$SCENARIO" == "5" ]]; then
    SAFE_FACTOR=$(echo "scale=1; $SAFE_FACTOR * 1.1" | bc)
fi
...
TCP_BUFFER_CONFIG="# TCP 缓冲区优化 (折中版 | ${BANDWIDTH}Mbps @ ${RTT_MS}ms)
...
echo -e "${BLUE}  折中安全系数：${SAFE_FACTOR}倍（平衡速度与重传）${NC}"
...
echo ""
echo -e "${YELLOW}6. 自动配置BBR+fq拥塞控制（分档降CPU）${NC}"
BBR_CONFIG=""
...
echo -e "  ✅ 折中BDP缓冲区：平衡带宽利用率与重传率"
echo -e "  ✅ BBR分档增益：低配平稳省CPU，高配激进跑满速"
