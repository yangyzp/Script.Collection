#!/bin/bash
# Linux 生产级高并发智能优化脚本 v2.5
# 核心：自动检测内存 → 智能推荐应用配置 | BDP自动计算 | 场景自适应
...
# 智能计算应用推荐值（永不超过系统上限）
NGINX_LIMIT=$(( TOTAL_MEM_MB * 100 ))
MYSQL_LIMIT=$(( TOTAL_MEM_MB * 50 ))
PHP_LIMIT=$(( TOTAL_MEM_MB * 100 ))
NGINX_LIMIT=$(( NGINX_LIMIT > SYSTEM_MAX_FILE ? SYSTEM_MAX_FILE : NGINX_LIMIT ))
MYSQL_LIMIT=$(( MYSQL_LIMIT > SYSTEM_MAX_FILE ? SYSTEM_MAX_FILE : MYSQL_LIMIT ))
PHP_LIMIT=$(( PHP_LIMIT > SYSTEM_MAX_FILE ? SYSTEM_MAX_FILE : PHP_LIMIT ))
...
# 标题
echo -e "${BLUE}=======================================================${NC}"
echo -e "${BLUE}  Linux 生产级智能优化脚本 v2.5 | 自动检测内存·智能调优${NC}"
echo -e "${BLUE}=======================================================${NC}"
echo ""
...
echo -e "系统最大文件句柄：${GREEN}${SYSTEM_MAX_FILE}${NC}"
echo -e "智能推荐应用配置已生成 ✅"
echo ""
...
echo -e "${GREEN}✅ 场景已选定：${SCENARIO_DESC}${NC}"
echo ""
...
echo -e "${YELLOW}🏗️  智能推荐应用层配置（根据 ${TOTAL_MEM_MB}MB 内存自动计算）${NC}"
echo -e "${GREEN}Nginx:${NC} worker_rlimit_nofile ${NGINX_LIMIT};"
echo -e "${GREEN}MySQL:${NC} open_files_limit = ${MYSQL_LIMIT}"
echo -e "${GREEN}PHP-FPM:${NC} rlimit_files = ${PHP_LIMIT}"
echo -e "${GREEN}Redis:${NC} maxclients 65535"
echo -e "${GREEN}Java:${NC} -XX:-MaxFDLimit"
echo ""
